...
class AutoSlugField(SlugField):
    ...
    def pre_save(self, instance, add):
        # Django 6.0+ calls pre_save twice during INSERT compilation
        # (once in save_base, again via the SQL compiler's pre_save_val).
        # Guard against re-processing the already-finalized slug — otherwise
        # non-idempotent custom slugify functions (e.g. one that replaces
        # '-' with '_') corrupt the value on the second call by mangling
        # the uniqueness suffix.
        sentinel = "_autoslug_inserting_" + self.attname
        if add and getattr(instance, sentinel, False):
        ...
        if add:
            setattr(instance, sentinel, True)
            post_save.connect(
                _clear_autoslug_sentinel,
                sender=type(instance),
                dispatch_uid="autoslug_clear_sentinel_" + self.attname,
                weak=False,
            )
...
def _clear_autoslug_sentinel(sender, instance, **kwargs):
    """Clear the per-save-cycle sentinel after the row is committed."""
    for attr in [
        a for a in list(instance.__dict__) if a.startswith("_autoslug_inserting_")
    ]:
